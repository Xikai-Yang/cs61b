...
public class BST<Key extends Comparable<Key>> {
    ...
    private int calDepth(Node x, Key key) {
        int cmp = key.compareTo(x.key);
        if (cmp == 0) {
            return 0;
        }
        if (cmp < 0) {
            return 1 + calDepth(x.left, key);
        }
        return 1 + calDepth(x.right, key);
    }
    private int calDepth(Key key) {
        return calDepth(root, key);
    }
    private int calTotalDepth(Node x) {
        if (x == null) {
            return 0;
        }
        return calDepth(x.key) + calTotalDepth(x.left) + calTotalDepth(x.right);
    }
    public int calTotalDepth() {
        return calTotalDepth(root);
    }
    public double calAvgDepth() {
        int totalDepth = calTotalDepth();
        double avg = (double) totalDepth/ (double)size();
        return avg;
    }
}
